...
let _keepAliveInitialized = false;
let _keepAliveJar = null;
let _keepAliveGlobalOptions = null;
...
async function loginHelper(appState, email, password, globalOptions, apiCustomized = {}, callback) {
  ...
  mainPromise
    .then(async (res) => {
      ...
      const appstatePath = require("path").resolve(__dirname, "../../../appstate.json");
      const saveAppState = () => {
        try {
          const currentState = api.getAppState();
          if (Array.isArray(currentState) && currentState.length > 0) {
            fs.writeFileSync(appstatePath, JSON.stringify(currentState, null, "\t"), "utf8");
          }
        } catch (saveErr) {
          logger.warn("failed to auto-save appstate: " + saveErr.message);
        }
      };

      if (!_keepAliveInitialized) {
        _keepAliveInitialized = true;
        _keepAliveJar = jar;
        _keepAliveGlobalOptions = globalOptions;

        setInterval(saveAppState, 2 * 60 * 1000);

        setInterval(async () => {
          try {
            await utils.get("https://www.facebook.com/", _keepAliveJar, null, _keepAliveGlobalOptions, { noRef: true });
            saveAppState();
          } catch (e) {
            logger.warn("cookie refresh ping failed: " + (e.message || e));
          }
        }, 30 * 60 * 1000);

        process.on("exit", saveAppState);
        process.on("SIGINT", saveAppState);
        process.on("SIGTERM", saveAppState);
      } else {
        _keepAliveJar = jar;
        _keepAliveGlobalOptions = globalOptions;
      }
